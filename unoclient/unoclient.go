...
type Option struct {
	Local bool // local machine

	ConvertTo     any      // convert option: --convert-to
	FilterName    any      // convert option: --filter, --output-filter
	FilterOptions []string // convert option: --filter-option, --filter-options
	UpdateIndex   bool     // convert option: --update-index
	InFilterName  any      // convert option: --input-filter

	FileType any // compare option: --file-type
}
...
func (uc *UnoClient) Convert(ctx context.Context, inData []byte, obs ...OptionBuilder) (outData []byte, err error) {
	...
	err = uc.call(ctx, "convert", &outData, nil, inData, nil,
		op.ConvertTo, op.FilterName, op.FilterOptions, op.UpdateIndex, op.InFilterName)
...
func (uc *UnoClient) ConvertFile(ctx context.Context, inFile, outFile string, obs ...OptionBuilder) (err error) {
	...
	if op.ConvertTo == nil || op.ConvertTo == "" {
...
func (uc *UnoClient) CompareFile(ctx context.Context, oldFile, newFile, outFile string, obs ...OptionBuilder) (err error) {
	...
	if op.FileType == nil || op.FileType == "" {
